...
export interface ApplyFlowDraftResult {
	...
	skipped: string[];
	/** Non-fatal support-file synchronization failures. */
	warnings: string[];
}

export type WorkbenchRecommendedAction =
	| "evolve-plan"
	| "evolve-show"
	| "evolve-apply"
	| "none";
...
export function listNewFlowEngines(): string[] {
	...
	return [
		...
		...registered
			.filter((name) => !PREFERRED_ENGINE_ORDER.includes(name))
			.sort(),
	];
}
...
export function effortFrontmatter(
	engine: string,
	effort: string,
...
export function docsReferenceSection(
	docs: readonly string[],
): string | undefined {
	const lines = docs
		.map((entry) => entry.trim())
		.filter(Boolean)
		.map(docReferenceLine);
...
function validateFlowId(flowId: string): void {
	if (!/^[A-Za-z0-9_-]+$/.test(flowId)) {
		throw new Error(
			"Flow identity may contain only letters, numbers, underscores, and hyphens.",
		);
	}
}
 ...
 */
export function draftFlowFromIntent(
	intent: string,
	options: DraftFlowOptions = {},
): FlowDraft {
	const normalized = normalizedIntent(intent);
	const slug =
		options.slug !== undefined
			? slugifyFlowIntent(options.slug)
			: suggestFlowSlug(normalized);
	const description = (options.description ?? defaultDescription(normalized))
		.replace(/\s+/g, " ")
		.trim();
	...
	const taskBody =
		options.body === undefined ? normalized : options.body.trim();
	...
	const frontmatter = yaml
		...
		.trimEnd();
	const base = `---\n${frontmatter}\n---\n\n${body}${body.endsWith("\n") ? "" : "\n"}`;
	const versioned = stampCreatedVersion(
		base,
		options.version ?? mdflowVersion(),
	);
...
export function applyFlowDraft(
	...
	const configContent = options.engine
		? projectConfig(options.engine)
		: undefined; // Validate before the first write.
	...
	if (options.withEval !== false) {
		try {
			if (
				writeExclusive(
					suitePath,
					renderEvalTemplate(inferEvalRecipes(draft.markdown)),
				) === "created"
			) {
	...
	try {
		const rosterResult = syncRosterReadme(target.projectRoot);
		if (!readmeExisted && rosterResult.changed) created.push(readmePath);
		if (rosterResult.state === "invalid")
			warnings.push(`Roster README was not updated: ${rosterResult.error}`);
	} catch (error) {
		warnings.push(
			`Roster README was not updated: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
	...
	if (currentConfig) {
		...
		if (writeExclusive(configPath, configContent) === "created")
			created.push(configPath);
		else skipped.push(configPath);
	}

	return { status: "created", target, flowPath, created, skipped, warnings };
}
...
export function summarizeFlowLifecycle(
	input: SummarizeFlowLifecycleInput,
): WorkbenchLifecycleSummary {
	const evidence = (input.evidence ?? []).filter(
		(item) => !input.flowId || item.flowId === input.flowId,
	);
	...
	const reviewable = latest
		? REVIEWABLE_EVOLUTION_STATUSES.has(latest.status)
		: false;
	let recommendedAction: WorkbenchRecommendedAction = "none";
	if (
		latest?.status === "verified_improvement" ||
		latest?.status === "regression_safe"
	) {
	...
	return {
		...
		evolution: {
			total: runs.length,
			...(latest
				? { latestRunId: latest.id, latestStatus: latest.status }
				: {}),
			active,
			reviewable,
		},
