...
export function renderEvalTemplate(recipes: EvalRecipe[] = ["output"]): string {
	const unique = [
		...new Set(recipes.length > 0 ? recipes : ["output" as EvalRecipe]),
	];
...
export function parseManagedEvalCases(source: string): ManagedEvalCaseParse {
	...
	const inScope = (index: number) =>
		...
		index > scopeStart &&
		(scopeEnd === undefined || index < scopeEnd);
	for (const [index, line] of lines.entries()) {
		...
		if (trimmed.startsWith(START_TOKEN)) {
			...
			if (!id)
				throw new Error(`line ${index + 1}: start marker has no case id`);
			if (open)
				throw new Error(
					`line ${index + 1}: nested case marker "${id}" inside "${open.id}"`,
				);
			if (seen.has(id))
				throw new Error(`line ${index + 1}: duplicate case id "${id}"`);
		...
		if (trimmed.startsWith(END_TOKEN)) {
			...
			if (!open)
				throw new Error(
					`line ${index + 1}: end marker "${id}" with no open case`,
				);
			if (open.id !== id) {
				throw new Error(
					`line ${index + 1}: end marker "${id}" does not close "${open.id}"`,
				);
			}
			seen.add(id);
			blocks.push({
...
export function insertEvalRecipes(
	...
	if (parsed.insertMarkerCount > 1) {
		throw new Error(
			`found ${parsed.insertMarkerCount} insertion markers; expected exactly one`,
		);
	}
...
export function inspectEvalSuiteStatic(
	...
	const draftFromPlan =
		plan?.cases.filter((item) => item.draft).map((item) => item.name) ?? [];
	const draftCaseIds = [
		...new Set([...detectDraftCaseIds(source), ...draftFromPlan]),
	];
	return {
		flowPath,
		suitePath,
...
export function classifyEvalVerdict(
	...
	if (input.draft) {
		return {
			verdict: "Unverified",
			current: false,
			reason: "draft assertions remain",
		};
	}
	...
	if (!entry.verification || !entry.lastRunFingerprint) {
		return {
			verdict: "Unverified",
			current: false,
			reason: "receipt is not fingerprint-bound",
		};
	}
	...
	if (entry.fail > 0) {
		return {
			verdict: "Failing",
			current: true,
			reason: `${entry.fail} behavioral case(s) failed`,
		};
	}
	...
	if (
		...
		return {
			verdict: "Unverified",
			current: true,
			reason: "receipt is incomplete or not clean",
		};
	}
	return {
		verdict: "Verified",
...
export async function resolveFlowPolicyRepetitions(
	flowPath: string,
): Promise<number> {
	try {
		...
		return resolveEvolutionPolicy(frontmatter.evolve ?? config.evolve)
			.repetitions;
	} catch {
		return 1;
	}
...
export async function inspectEvalStatus(
	...
	options: {
		...
		/** Pre-resolved by strictly static callers that must not derive Git identity. */
		entry?: EvalLedgerEntry | null;
	} = {},
): Promise<EvalStatus> {
	...
	const policyRepetitions =
		options.policyRepetitions ?? (await resolveFlowPolicyRepetitions(absolute));
	...
	const entry =
		options.entry !== undefined
			? (options.entry ?? undefined)
			: inspection.exists
				? getEvalLedgerEntry(inspection.suitePath, ledger)
				: undefined;
	...
	if (
		...
		try {
			...
			currentFingerprint = (
				await buildVerificationFingerprint(
					absolute,
					inspection.suitePath,
					cases,
				)
			).fingerprint;
		} catch {
	...
	return {
		...
		lastFullRunAt:
			entry?.lastFullRunAt ?? (entry?.full ? entry?.lastRunAt : undefined),
...
function listFlowsRecursive(dir: string, out: string[] = []): string[] {
	for (const entry of readdirSync(dir, { withFileTypes: true })) {
		...
		if (entry.isDirectory()) {
			if (!COVERAGE_SKIP_DIRS.has(entry.name.toLowerCase()))
				listFlowsRecursive(path, out);
			continue;
		}
...
export async function inspectEvalCoverage(
	...
	const baselined = uncovered.filter(
		(path) => path in (baseline.uncovered ?? {}),
	);
	...
	for (const path of baselineEntries) {
		if (!flowSet.has(path)) {
			zombies.push({
				path,
				reason: "baseline entry names no scanned flow — remove it",
			});
		} else if (covered.includes(path)) {
			zombies.push({
				path,
				reason: "flow now has a valid suite — remove its baseline entry",
			});
		}
	}
	for (const zombie of zombies)
		failures.push(`zombie baseline entry: ${zombie.path} — ${zombie.reason}`);
...
export function readEvalCoverageBaseline(path: string): EvalCoverageBaseline {
	let raw: Partial<EvalCoverageBaseline>;
	try {
		raw = JSON.parse(
			readFileSync(path, "utf8"),
		) as Partial<EvalCoverageBaseline>;
	} catch (error) {
		throw new Error(
			`${basename(path)}: invalid JSON (${error instanceof Error ? error.message : String(error)})`,
		);
	}
	if (
		raw.schemaVersion !== 1 ||
	...
	for (const [flow, reason] of Object.entries(raw.uncovered)) {
		if (typeof reason !== "string" || !reason.trim()) {
			throw new Error(
				`${basename(path)}: baseline entry "${flow}" needs a non-empty string reason`,
			);
		}
		const invalid =
			!/\.md$/i.test(flow) ||
	...
	return {
		schemaVersion: 1,
		uncovered: raw.uncovered as Record<string, string>,
	};
}
