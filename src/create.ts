...
export function parseCreateArgs(args: readonly string[]): CreateOptions {
	...
	if (
		intentParts.length === 1 &&
		/\.md$/i.test(intentParts[0]!) &&
		!options.name
	) {
...
function mergeFrontmatter(
	...
	const frontmatter = yaml
		.dump({ ...existing, ...extra }, { lineWidth: -1, noRefs: true })
		.trimEnd();
...
export async function runCreate(
	...
	const promptIntent =
		...
		(() =>
			tabSafeInput({
				...
				validate: (value) =>
					value.trim().length > 0 || "Describe the flow in a few words",
			}));
	const intent =
		options.intent?.trim() ||
		legacyName?.intent ||
		(await promptIntent()).trim();
	if (!intent) throw new Error("Flow intent cannot be empty.");

	const engine = options.engine?.trim() || legacyName?.engine;
	if (options.engine !== undefined && !engine)
		throw new Error("Create engine cannot be empty.");
	...
	if (options.withEval && !created.includes(suitePath)) {
		try {
			const { inferEvalRecipes, renderEvalTemplate } = await import(
				"./eval-convention"
			);
			...
			try {
				...
				for (const path of [
					flowPath,
					...created.filter((item) => item !== flowPath),
				]) {
					try {
						unlinkSync(path);
					} catch {}
	...
	if (options.location === "user") {
		...
		await (runtime.ensureGlobalAvailability ?? ensureGlobalFlowAvailable)(
			availability,
		);
	}
	...
	const supportFiles = created.filter(
		(path) => path !== flowPath && path !== suitePath,
	);
	if (supportFiles.length > 0)
		log(`Added project support: ${supportFiles.join(", ")}`);
	for (const message of applyWarnings) warn(`Warning: ${message}`);
	if (canonical) {
		log("Agent guide: flows/README.md");
		log("Inspect: md doctor --json");
	}
	if (options.location === "user") {
		log(`Run from any directory: ${scope.invocation}`);
		log(
			`Exact global path (bypasses a same-named project flow): ${scope.exactInvocation}`,
		);
	} else if (options.location === "cwd") {
