...
- Begin every maintenance task with `md doctor --json`; it is a read-only, no-execution project inspection whose stable diagnostic codes and effect-labelled next actions are authoritative for the current checkout.
...
6. __Hand off.__ Print the final roster and the trust loop: run the flow, record concrete failures with `md feedback`, turn feedback into reviewed eval cases, run `md eval <flow> --plan` before any paid verification, then use `md evolve plan` → `propose` → `show` → explicit `apply`/`reject`. Tell them how to do their first real run and actual eval run, including their separate costs and confirmation boundaries.
