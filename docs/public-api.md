...
| `md roster sync [--check] [--agents] [--json]` | Synchronize only mdflow's marked operator-card block in `flows/README.md`, preserving all text outside the markers. Sync is `LOCAL WRITE`; `--check` is `FREE`, never writes, and exits 1 when stale or invalid. `--agents` is the explicit flows-first opt-in: it creates or extends one marker-managed mdflow block in `AGENTS.md` and `CLAUDE.md` at the project root. Plain sync refreshes those blocks only in files that already carry the markers and never adds them. |
